...
osnovne vrste struktura u Pythonu: popis (`list`{.python}) i rječnik
(`dict`{.python}). Učenjem ovih koncepata približavamo se i temeljima
razmjene podataka u mnogim web uslugama. Tome možemo dodati i skup
(`set`{.python}) koji se nešto manje koristi od popisa i rječnika, ali
je u nekim slučajevima vrlo korisna struktura.
...
`list`{.python}. Ovako definiran popis se može mijenjati i vjerojatno je
najčešće korištena struktura podataka u Python programima. Pogledajmo
kako možemo stvoriti popis i dohvatiti neki element iz njega.
...
direktoriju. U prikazanom slučaju na disku `c`{.python} postoji
direktorij `test`{.python} koji sadrži tri datoteke
(`dat_a.pdf`{.python}, `dat_b.docx`{.python} i `dat_c.txt`{.python}) i
dva poddirektorija (`dir_a`{.python} i `dir_b`{.python}). Jednostavno
dobiti popis članova nekog direktorija, kao što vidimo, nije teško.
Ipak, dok je samo po sebi jasno kako bi ovakav popis mogao biti
koristan, ima još puno posla kako bi s ovime mogli učiniti nešo
konkretno: razlikovati datoteke od direktorija, izraditi pune putanje od
imena, razlikovati različite vrste datoteka, pristupati sadržaju
datoteke i slično. Drugim riječima, problem prvog dodira sa strukturama
podataka je što su one vrlo važne za programiranje gotovo bilo čega, ali
primjeri koji bi prikazali njihovu praktičnu uporabu su često preopširni
ili prekompleksni za prvi susret. Ovaj dio skripte će se, stoga,
usredotočiti na jednostavne apstraktne primjere rada sa strukturama
podataka, a njihova praktična uporaba će se prikazati u kasnijim većim
primjerima koji rade nešto konkretno ili zabavno.

##### Funkcija `list`{.python}

Kao i kod tzv. primitivnih vrsti vrijednosti u Pythonu (`int`{.python},
`float`{.python}, `bool`{.python}, `str`{.python}), nazivi struktura
vrijednosti su također i funkcije s kojima možemo druge objekte
pretvarati u tu strukturu. Funkcija `list`{.python} prima jedan
parametar i raščlanjuje poslanu vrijednost na sastavne elemente te vraća
rezultat kao popis.
...
kaže \"objekt vrste int nije iterativan\". Sjetimo se, kada je
vrijednost \"iterativna\", to znači da se po njoj ne može prebirati što
je upravo zahtjev da bi se nešto moglo pretvoriti u popis. Vrijednost
...
vrijednostima popisa. U tu svrhu, vrlo nam je korisna funkcija
`range`{.python}. Ta funkcija služi stvaranju niza brojeva od nekog
početnog do nekog završnog. Pogledajmo primjere:
...
Kao što vidimo, funkcija `range`{.python} posebno je korisna za
generiranje validnih indeksa za popis. Kao i kod indeksa popisa, ova
funkcija uključuje prvu vrijednost, a zadnju ne. Na primjer,
`range(0, 4)`{.python} generira brojeve 0, 1, 2 i 3. Ukoliko traženi
brojevi počinju od 0, funkcija range se može pozvati sa samo jednim
parametrom koji označava do kojeg broja se generira. Ovo nam omogućuje
da ukoliko želimo generirati sve validne indekse za neki popis to možemo
učiniti jednostavnim izrazom `range(len(neki_popis))`{.python}.

Osim definiranja minimuma i maksimuma, `range`{.python} prima i
opcionalni treći parametar: korak (eng. *step*). Ovaj parametar je zadan
na 1 i definira pomak između dva broja koja se generiraju funkcijom
`range`{.python}. Navedeno je možda teže shvatiti kroz definiciju nego
kroz primjer pa pogledajmo jedan:
...
Do sada smo radili samo s popisom vrste `list`{.python}, ali to nije
jedina vrsta popisa. Glavna distinkcija između popisa u Pythonu ovisi o
tome da li je popis promjenjiv ili nepromjenjiv. `list`{.python} je, kao
što smo vidjeli iz primjera, promjenjiv popis. Postoji i nepromjenjivi
popis koji se naziva `tuple`{.python}. Dohvaćanje elemenata iz obje
vrste popisa je identično, ali `tuple`{.python} se ne definira kroz
uglate već kroz oble zagrade, a u mnogim slučajevima se može pisati i
bez zagrada.

Kao što vidimo, razlika između prikazane dvije vrste popisa je u tome
što je `list`{.python} moguće mijenjati, a `tuple`{.python} ne. Sve što
možemo raditi s popisom vrste `tuple`{.python}, dakle, vrijedi i za
popise vrste `list`{.python}, ali `list`{.python} pruža mogućnost
promjena u popisu, a `tuple`{.python} ne. Pokušaj dodavanja nove
vrijednosti u `list`{.python} je, na primjer, normalan postupak, a
pokušaj dodavanja nove vrijednosti u `tuple`{.python} javlja grešku jer
je tuple nepromjenjiv. S druge strane, dohvaćanje vrijednosti putem
indeksa funkcionira identično i za `list`{.python} i za
`tuple`{.python}.
...
načinu implementacije popisa. Pythonov `list`{.python}, na primjer, je
visoko efikasan u dodavanju nove vrijednosti na kraj popisa kao i u
izbacivanju te vrijednosti, ali neefikasan u dodavanju vrijednosti na
početak popisa ili izbacivanja vrijednosti s početka popisa. Da
situacija bude još gora, dodavanje odnosno izbacivanje vrijednosti je
tim manje efikasno čim ima više objekata u popisu nakon objekta koji se
dodaje ili izbacuje.
...
dostupna iz modula `collections`{.python} i zove se `deque`{.python}.
Zašto onda ne bismo uvijek koristili `deque`{.python}? Zato jer za
dodatne mogućnosti uvijek postoji cijena pa je tako popis vrste
`deque`{.python} manje memorijski efikasan od popisa vrste
`list`{.python}. U svakom slučaju, modul `collections`{.python}, kao što
ime kaže, donosi dodatne strukture podataka koje su specijaliziranije u
...
Osim spomenutih posebnih metoda koje dozvoljava struktura
`list`{.python}, postoje i dva općenita pitanja koja često želimo
postaviti bilo kojoj strukturi podataka. Ta pitanja su:
...
strukture. Python funkcija `len`{.python} vraća upravo taj broj.
...
`len`{.python} je funkcija koja radi na svim vrstama objekata za koje je
pitanje \"Koliko ima elemenata (tj. podobjekata) u ovom objektu?\"
validno. Ako pitanje nije validno, odnosno objekt koji je poslan kao
parametar ga ne podržava, `len`{.python} javlja grešku.

Kao što je za očekivati, poziv funkcije `len`{.python} s cijelim brojem
kao parametrom javlja grešku koju možemo čitati kao \"cijeli brojevi
nemaju broj elemenata\". `len`{.python} teksta, međutim, vraća
vrijednost jer je kod teksta to \"broj znakova\".

##### Provjera članstva neke strukture

Također, postoji univerzalan način provjere da li se neka vrijednost
nalazi u određenoj zbirci vrijednosti i to putem operatora
`in`{.python}. Pogledajmo primjere:
...
``` {#listing:dict_prijevod .python caption="Korištenje rječnika za prevođenje" label="listing:dict_prijevod"}
```
...
Također, primijetimo da prebiranje po rječniku s petljom `for`{.python}
prebire po ključevima. Što ako želimo prebirati po vrijednostima ili po
...
Funkcija `dict`{.python} služi stvaranju rječnika. Kao i sve ostale
funkcije koje označavaju vrstu vrijednosti, poziv bez parametara stvara
prazan rječnik.
...
Poziv na `dict`{.python} s imenovanim parametrima stvara novi rječnik s
tim parametrima kao ključevima.
...
rječnika je što se skup ne sastoji od \"ključ: vrijednost\" parova, već
od individualnih elemenata. Nažalost, obzirom da se vitičaste zagrade
...
\"redak\" i \"stupac\". Navedeno predstavlja općenito viđenje podataka
koje želimo postići ovom skriptom te povezati s programskim konceptima.
Ipak, krenimo prvo s poznatim konceptima svojstvenima tablici, a razlog
...
autori se nalaze u drugom stupcu\". Jedan način na koji možemo
definirati tablicu je popis redaka[^3]. Redak možemo definirati kao
popis vrijednosti u dogovorenom redoslijedu koji je zadan naslovima
...
``` {#listing:popis_redak caption="Zaglavlje i podatkovni redak kao popisi" label="listing:popis_redak"}
...
``` {#listing:popis_tablica caption="Zaglavlje i tablica kao popisi" label="listing:popis_tablica"}
zaglavlje = ['Naslov', 'Autor', 'Godina', 'Izdavač', 'ISBN']
tablica = [
    ['Good Omens',  'Terry Pratchett & Neil Gaiman',  1990, 'Gollancz', '0-575-04800-X'],
    ['Interesting times',  'Terry Pratchett',  1994, 'Gollancz', '0-575-05800-5'],
...
``` {#listing:popis_tablica_zaglavlje caption="Tablica koja sadrži zaglavlje" label="listing:popis_tablica_zaglavlje"}
tablica = [
    ['Naslov', 'Autor', 'Godina', 'Izdavač', 'ISBN']
    ['Good Omens',  'Terry Pratchett & Neil Gaiman',  1990, 'Gollancz', '0-575-04800-X'],
    ['Interesting times',  'Terry Pratchett',  1994, 'Gollancz', '0-575-05800-5'],
...
odvajati \"shemu podataka\" ili \"metapodatke\" od samih \"podataka o
nečemu\" jer ćemo tako imati znantno manje problema u kasnijoj obradi i
pohrani ovakvih podataka.
...
``` {#listing:tablica_dohvat caption="Dohvat vrijednosti iz popisa popisa" label="listing:tablica_dohvat"}
...
``` {#listing:tablica_dohvat2 caption="Dohvat vrijednosti iz popisa popisa s razrješavanjem naziva" label="listing:tablica_dohvat2"}
...
``` {#listing:rjecnik_popisa caption="Tablica kao rječnik popisa" label="listing:rjecnik_popisa"}
zaglavlje = ['Naslov', 'Autor', 'Godina', 'Izdavač', 'ISBN']
tablica = {
...
primjeru
[\[listing:rjecnik_popisa_dohvat\]](#listing:rjecnik_popisa_dohvat){reference-type="ref"
reference="listing:rjecnik_popisa_dohvat"}.

``` {#listing:rjecnik_popisa_dohvat caption="Dohvat vrijednosti iz rječnika popisa" label="listing:rjecnik_popisa_dohvat"}
...
[\[listing:pp_u_rp1\]](#listing:pp_u_rp1){reference-type="ref"
reference="listing:pp_u_rp1"}.

``` {#listing:pp_u_rp1 caption="Popis popisa -> rječnik popisa, zadrži šifru" label="listing:pp_u_rp1"}
zaglavlje = ['Naslov', 'Autor', 'Godina', 'Izdavač', 'ISBN']
tablica = [
    ['Good Omens',  'Terry Pratchett & Neil Gaiman',  1990, 'Gollancz', '0-575-04800-X'],
    ['Interesting times',  'Terry Pratchett',  1994, 'Gollancz', '0-575-05800-5'],
...
[\[listing:pp_u_rp2\]](#listing:pp_u_rp2){reference-type="ref"
reference="listing:pp_u_rp2"} :

``` {#listing:pp_u_rp2 caption="Popis popisa -> rječnik popisa, stvori šifru" label="listing:pp_u_rp2"}
...
ključ, a koji identificira knjigu \"Neverwhere\" u nekom njezinom
obliku. Možda i želimo da jedan naš \"redak\" sadrži sve ISBN-ove koje
se prema našim kriterijima odnose na isti entitet odnosno, u ovom
slučaju, djelo. Ako je tako, tada bi također dodijelili svoje šifre kao
u prijašnjem primjeru.
...
\"stupaca u tablici\"), a vrijednosti u rječniku su upravo vrijednosti
tih svojstava.

``` {#listing:redak_rjecnik caption="Redak kao rječnik" label="listing:redak_rjecnik"}
redak = {
...
``` {#listing:podaci_rjecnik caption="Podaci kao rječnik rječnika" label="listing:podaci_rjecnik"}
tablica_rjecnik = {
    '0-575-05800-5': {
       'Naslov': 'Interesting times', 
       'Autor': 'Terry Pratchett',
...
``` {#listing:popis_u_rjecnik caption="Popis vrijednosti -> rječnik vrijednosti" label="listing:popis_u_rjecnik"}
# ulazni podaci
zaglavlje = ['Naslov', 'Autor', 'Godina', 'Izdavač', 'ISBN']
...
[\[listing:pp_u_rp2\]](#listing:pp_u_rp2){reference-type="ref"
reference="listing:pp_u_rp2"} i
[\[listing:popis_u_rjecnik\]](#listing:popis_u_rjecnik){reference-type="ref"
reference="listing:popis_u_rjecnik"}.

``` {#listing:pp_u_rr caption="Popis popisa -> rječnik rječnika" label="listing:pp_u_rr"}
# ulazni podaci
zaglavlje = ['Naslov', 'Autor', 'Godina', 'Izdavač', 'ISBN']
tablica = [
    ['Good Omens',  'Terry Pratchett & Neil Gaiman',  1990, 'Gollancz', '0-575-04800-X'],
...
\"autor\". \"Terry Pratchett & Neil Gaiman\" nije neobična vrijednost
budući da neko djelo može imati više autora. To nam samo po sebi već
govori da je vrijednost atributa \"autori\" zapravo struktura
vrijednosti i to popis autora (odnosno stringova).

Pogledajmo kako postaviti naš bibliografski zapis za neku knjigu kako bi
dopuštao koautorstvo.

``` {#listing:ugnj1 caption="Ugniježđeni podaci 1" label="listing:ugnj1"}
zapis = {
   'Naslov': 'Good Omens', 
...
``` {#listing:ime caption="Struktura individualnog osobnog imena" label="listing:ime"}
osoba = {
    'ime': 'Terry', 
    'prezime': 'Pratchett'
}
...
``` {#listing:ugnj2 caption="Ugnježđeni podaci 2" label="listing:ugnj2"}
zapis = {
    'Naslov': 'Good Omens', 
    'Autor': [
...
imena. Sada nam je, na primjer, lakše generirati stringove u obliku
\"Ime Prezime\", \"Prezime, Ime\" ili \"Prezime, I.\" za različite
potrebe.
